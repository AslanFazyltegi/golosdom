...
export function InfocenterNewsPage({ activeRole, refreshCommunicationUnreadCounts }: CabinetModuleProps) {
  ...
  if (activeRole !== "CHAIRMAN") {
    return <NewsViewer refreshCommunicationUnreadCounts={refreshCommunicationUnreadCounts} />;
  }
...
function NewsViewer({
  ...
  async function load() {
    ...
    setError("");
    try {
      setItems(await fetchMyInfocenterNews());
    } catch (err) {
  ...
  useEffect(() => {
    ...
    fetchMyInfocenterNews()
      ...
      .catch((err) => {
        if (!cancelled) setError(err instanceof Error ? err.message : "Не удалось загрузить новости.");
      })
      .finally(() => {
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      if (filter === "important" && !item.is_important) return false;
      if (!query) return true;
      return [item.title, item.summary, item.category, stripHtml(item.body_html)]
        .join(" ")
        .toLowerCase()
        .includes(query);
    });
  ...
  async function openItem(item: InfocenterNews) {
    let nextItem = item;
    setDetails(item);
    if (!item.read_at) {
      try {
        await markInfocenterNewsRead(item.id);
        ...
        setError(err instanceof Error ? err.message : "Не удалось отметить новость прочитанной.");
      }
    }
  }
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-6 text-slate-900">
      ...
      {details && <ViewerNewsDetails item={details} onClose={() => setDetails(null)} />}
    </main>
  );
}
...
function ViewerNewsCard({ item, onOpen }: { item: InfocenterNews; onOpen: () => void }) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2 text-xs font-semibold">
            ...
            {item.is_important && <span className="rounded-full bg-red-50 px-2 py-1 text-red-700">Важная</span>}
            {item.read_at ? (
...
function ViewerNewsDetails({ item, onClose }: { item: InfocenterNews; onClose: () => void }) {
  const cover = coverImage(item);
  return (
    <Modal title="Новость" onClose={onClose} wide>
      <article>
        {cover && <img src={infocenterNewsMediaUrl(cover.file_url)} alt="" className="max-h-80 w-full rounded-2xl object-cover" />}
        <p className="mt-4 text-sm font-semibold text-blue-600">{item.category}</p>
        <h2 className="mt-2 text-3xl font-bold">{item.title}</h2>
        <p className="mt-2 text-sm text-slate-500">Дата публикации: {item.published_at ? formatAstanaDateTime(item.published_at) : "Не указана"}</p>
